...
function generateMobileToken($user, int $ttl = 2592000): string
{
    ...
    $secret_key = defined('TURN_SECRET') && TURN_SECRET !== '' ? TURN_SECRET : DB_PASS;
...
function validateMobileToken(?string $token): ?array
{
    ...
    // DB user check
    $db = getDB();
    $stmt = $db->prepare('SELECT id, username, full_name, role, extension, extension_type, is_active FROM sys_users WHERE id = ?');
